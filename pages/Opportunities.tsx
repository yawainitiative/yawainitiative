...
import { Briefcase, GraduationCap, Coins, ExternalLink } from 'lucide-react';

const Opportunities: React.FC = () => {
  const [filter, setFilter] = useState('All');
  const tabs = ['All', 'Job', 'Scholarship', 'Grant', 'Internship'];
      ...
      <div className="space-y-4">
        {filteredOpp.map((item) => (
          <div key={item.id} className="group bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:shadow-xl hover:border-yawai-gold/30 transition-all duration-300">
             ...
             <button className="w-full md:w-auto bg-slate-50 text-slate-700 px-8 py-3 rounded-xl font-bold hover:bg-yawai-gold hover:text-white transition-all shadow-sm hover:shadow-lg flex items-center justify-center gap-2">
        ...
        {filteredOpp.length === 0 && (
          <div className="text-center py-20 text-slate-400">
             <div className="inline-block p-4 bg-slate-100 rounded-full mb-4"><Briefcase size={32} /></div>
             <p className="font-medium">No opportunities found in this category.</p>
          </div>
        )}
      </div>
    </div>
